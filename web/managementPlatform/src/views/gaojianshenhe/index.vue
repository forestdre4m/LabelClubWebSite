<template>
	<div>
		<el-form :inline="true" :model="formInline" class="user-search" style="float:right">
			<el-form-item label=""><el-input size="small" style="width: 100px;" v-model="formInline.author" placeholder="输入投稿人"></el-input></el-form-item>
		...
		<el-table size="small" :data="listData" highlight-current-row v-loading="loading" border element-loading-text="拼命加载中" @selection-change="handleSelectionChange">
			...
			<el-table-column sortable prop="author" label="投稿人"></el-table-column>
			...
			<el-table-column v-if="userInfo.roles[0] == 'changpai'" align="center" sortable prop="issh" label="是否审核" >
			...
			<el-table-column align="center" label="操作" min-width="160">
				<template slot-scope="scope">
					...
					<!--liangbuan-->
					<el-button size="mini" type="success" icon="el-icon-edit" @click="handleEdit(scope.$index, scope.row)" v-if="userInfo.roles[0] == 'guanliyuan'  || userInfo.roles[0] == 'changpai'"></el-button>
					<el-button size="mini" type="danger" icon="el-icon-delete" @click="deleteGaojianshenhe(scope.$index, scope.row)" v-if="userInfo.roles[0] == 'guanliyuan'  || userInfo.roles[0] == 'changpai'"></el-button>
		...
		<el-form-item  label="作品名" prop="audioname"><el-input size="small" v-model="editForm.audioname" auto-complete="off" placeholder="请输入作品名" style='width:50%' disabled></el-input></el-form-item>
			...
			<table width="100%" border="0"  align="center" cellpadding="3" cellspacing="1" class="table table-striped table-bordered table-hover">
								<tr>
									<td width='11%' height='40px'>投稿人：</td><td width='39%'>{{editForm.author}}</td>
...
import { gaojianshenheList, gaojianshenheSave, gaojianshenheDelete,gaojianshenheEdit,gaojianshenheDeleteList } from '@/api/gaojianshenhe/gaojianshenheApi';
...
export default {
	...
	methods: {
		handleShenheList(){
				const gaojianshenhes = this.gaojianshenhes;
				if(gaojianshenhes.length == 0){
					this.$message({
						type: 'error',
						message: '请至少选择一项'
					})
					...
					this.$confirm('确定要批量审核吗?', '信息', {
						...
						gaojianshenhes.forEach(item=> {
							let data = {...item, issh: 1}
							gaojianshenheEdit(data).then(res => {})
						})
						this.getdata(this.formInline)
						this.$message({
							type: 'success',
		...
		submitForm(editData) {
			...
			this.$refs[editData].validate(valid => {
				if (valid) {
					if(this.editForm.id){
						gaojianshenheEdit(this.editForm).then(res => {
		...
		// 删除公司
		deleteGaojianshenhe(index, row) {
			this.$confirm('确定要删除吗?', '信息', {
				confirmButtonText: '确定',
				cancelButtonText: '取消',
				type: 'warning'
			}).then(() => {
				gaojianshenheDelete(row.id).then(res => {
					if (res.code == '0') {
						this.$message({
								type: 'success',
							message: '稿件审核已删除!'
		...
		handleDeleteList(){
			...
			if(gaojianshenhes.length == 0){
				...
				this.$confirm('确定要批量删除吗?', '信息', {
					...
					gaojianshenheDeleteList(gaojianshenhes).then(res => {
						if (res.code == '0') {
							this.$message({
								type: 'success',
								message: '批量删除成功!'
