...
export function changpaiUser(router, to, next) {
	resetRouter();
	menuApi
		.getMenuChangpai()
		.then(async (res) => {
...
export function yinlerenUser(router, to, next) {
	resetRouter();
	menuApi
		.getMenuYinleren()
		.then(async (res) => {
...
export function getRouterList(router, to, next) {
	...
	if (Session.get('userInfo').roles[0] === 'changpai') changpaiUser(router, to, next);
	if (Session.get('userInfo').roles[0] === 'yinleren') yinlerenUser(router, to, next);
